...
main() {
    ...
    if ! docker image inspect "$core_image" >/dev/null 2>&1; then
        ...
        # Include custom-claude assets for core image (commands/hooks/scripts/config)
        if [[ -d "${root_dir}/custom-claude" ]]; then
            rsync -a --delete "${root_dir}/custom-claude/" "$build_context/custom-claude/" 2>/dev/null || cp -R "${root_dir}/custom-claude" "$build_context/custom-claude"
        fi

        # Normalize line endings (CRLF->LF) for all shell/entrypoint scripts in build context (core image)
        if command -v sed >/dev/null 2>&1; then
            while IFS= read -r -d '' f; do sed -i 's/\r$//' "$f" 2>/dev/null || true; done < <(find "$build_context" -maxdepth 3 -type f \( -name '*.sh' -o -name 'docker-entrypoint.sh' -o -name 'init-firewall' -o -name 'generate-tools-readme' -o -name 'tools-report.sh' \) -print0 2>/dev/null)
        fi
...
build_docker_image() {
    ...
    # Include custom-claude directory if present (commands/hooks/scripts/config)
    if [[ -d "${root_dir}/custom-claude" ]]; then
        rsync -a --delete "${root_dir}/custom-claude/" "$build_context/custom-claude/" 2>/dev/null || cp -R "${root_dir}/custom-claude" "$build_context/custom-claude"
    fi

    # Normalize line endings (CRLF->LF) for project image build context
    if command -v sed >/dev/null 2>&1; then
        while IFS= read -r -d '' f; do sed -i 's/\r$//' "$f" 2>/dev/null || true; done < <(find "$build_context" -maxdepth 3 -type f \( -name '*.sh' -o -name 'docker-entrypoint.sh' -o -name 'init-firewall' -o -name 'generate-tools-readme' -o -name 'tools-report.sh' \) -print0 2>/dev/null)
    fi
